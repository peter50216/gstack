...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(jj root 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
If A:
1. Remove the vendored directory from the working copy: `rm -rf .factory/skills/gstack/`
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(jj log -r @ --no-graph -T 'commit_id.short()' 2>/dev/null)"'","via":"ship"}'
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(jj log -r @ --no-graph -T 'commit_id.short()' 2>/dev/null)"'"}'
```
...
**Move the bookmark to the latest committed change.** After `jj commit` in Step
15.1, the working copy `@` is a fresh empty change on top of the just-created
commit. Bookmarks don't auto-advance in jj — they must be explicitly created or
moved to `@-` before push, or `jj git push` will push stale or empty state.

```bash
# Create or move bookmark to @- (the latest committed change).
# --allow-backwards covers the case where a prior aborted /ship left the
# bookmark ahead of @-.
jj bookmark set <branch-name> -r @- --allow-backwards
```

**Idempotency check:** Compare the bookmark to its remote-tracked counterpart.

```bash
jj git fetch --remote origin --branch <branch-name> 2>/dev/null
LOCAL=$(jj log -r '<branch-name>' --no-graph -T 'commit_id.short()' 2>/dev/null || echo "none")
REMOTE=$(jj log -r '<branch-name>@origin' --no-graph -T 'commit_id.short()' 2>/dev/null || echo "none")
echo "LOCAL: $LOCAL  REMOTE: $REMOTE"
[ "$LOCAL" = "$REMOTE" ] && [ "$LOCAL" != "none" ] && echo "ALREADY_PUSHED" || echo "PUSH_NEEDED"
```

If `ALREADY_PUSHED`, skip the push but continue to Step 18. Otherwise push:
