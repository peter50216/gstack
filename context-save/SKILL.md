...
version: 1.0.0
description: |
  Save working context. Captures VCS state, decisions made, and remaining work
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"context-save","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(jj root 2>/dev/null || git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
echo "=== RECENT LOG ==="
jj log -n 10 --no-graph 2>/dev/null
```
...
The `files_modified` list comes from `jj status`. Use relative paths from the repo root.
...
- **Infer, don't interrogate.** Use jj state and conversation context to fill in
