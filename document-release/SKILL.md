...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(jj root 2>/dev/null || git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
```bash
jj diff --from <base>@origin --to @ --stat
```
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./.jj/*" -not -path "./node_modules/*" -not -path "./.gstack/*" -not -path "./.context/*" | sort
```
...
```bash
jj diff --from <base>@origin --to @ -- VERSION
```
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   b. Read the full diff (`jj diff --from <base>@origin --to @ --stat` and `jj diff --from <base>@origin --to @ --name-only`).
...
**Empty check first:** Run `jj status`. If no documentation files were
modified by any previous step, output "All documentation is up to date." and exit without
committing.

**Commit:**

1. Commit modified documentation files by name.
2. Create a single commit:

```bash
jj commit <doc-files> -m "$(cat <<'EOF'
